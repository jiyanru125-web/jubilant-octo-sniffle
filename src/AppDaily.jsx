...
function label(names) { return names.length > 1 ? `${names.length} 张照片` : names[0] || ''; }
function toTask(row) {
  const names = parseList(row.proof_name);
  const urls = parseList(row.proof_url);
  return { id: row.id, title: row.title, done: !!row.done, proofNames: names, proofUrls: urls, proofName: label(names), proofUrl: urls[0] || '' };
}
...
function status(percent, hasTask = true) {
  if (!hasTask) return { text: '暂无任务', cls: 'bg-slate-100 text-slate-500', dot: 'bg-slate-300' };
...
function makeHistory(rows, currentDate) {
  ...
  PLAYERS.forEach((p) => {
    out[p.name] = ds.map((day) => {
      ...
      return { ...pretty(day), taskCount: tasks.length, doneCount: done, percent: tasks.length ? Math.round(done / tasks.length * 100) : 0 };
    });
  });
  return out;
}
function streak(rows) {
  ...
  for (let i = rows.length - 1; i >= 0; i--) {
    if (rows[i].taskCount > 0 && rows[i].percent === 100) n++;
    else break;
  }
  return n;
}
function weekReports(history) {
  ...
  return base.map((d, i) => ({ d, i })).filter(({ d, i }) => d.dow === 0 || i === base.length - 1).slice(-5).map(({ i }) => {
    ...
    const players = PLAYERS.map((p) => {
      ...
      return { ...p, activeDays: rows.length, fullDays: rows.filter((r) => r.percent === 100).length, avg };
    });
...
export default function IELTSBattleRoom() {
  ...
  async function addTask(player) {
    const title = taskDrafts[player].trim(); if (!title) return;
    setTaskDrafts((c) => ({ ...c, [player]: '' }));
    ...
    await loadData(); setStatusMessage('任务已修改，以后每天会沿用新版任务。');
  }
  async function uploadProof(player, id, files) {
    if (!files.length) return;
    const today = todayKey();
    const taskTitle = (taskLists[player] || []).find((t) => t.id === id)?.title || '学习任务';
    const names = files.map((f) => f.name); const localUrls = files.map((f) => URL.createObjectURL(f)); const proofName = label(names);
    ...
    for (const file of files) {
      const path = `${today}/${playerSlug(player)}/${id}/${safeName(file.name)}`;
      const uploaded = await supabase.storage.from('task-proofs').upload(path, file, { upsert: false, contentType: file.type || 'image/jpeg' });
      if (uploaded.error) { setStatusMessage(`照片上传失败：${uploaded.error.message}`); updateLocal(player, id, (t) => ({ ...t, done: false, proofName: '', proofNames: [], proofUrl: '', proofUrls: [] })); return; }
      const { data } = supabase.storage.from('task-proofs').getPublicUrl(path); urls.push(data.publicUrl);
    }
  ...
  const stats = useMemo(() => PLAYERS.map((p) => { const tasks = taskLists[p.name] || []; const done = tasks.filter((t) => t.proofName).length; const percent = tasks.length ? Math.round(done / tasks.length * 100) : 0; return { ...p, doneCount: done, taskCount: tasks.length, percent, streak: streak(history[p.name] || []) }; }), [taskLists, history]);
